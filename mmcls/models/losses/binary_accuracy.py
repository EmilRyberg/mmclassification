...
def accuracy_torch(pred, target, thrs=0.5):
    res = []
    num = pred.size(0)
    pred_score = pred
    pred_label = (pred_score > thrs).int()
...
def binary_accuracy(pred, target, thrs=0.5):
    ...
    res = accuracy_torch(pred, target, thrs)
...
class BinaryAccuracy(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        """
        return binary_accuracy(pred, target)
